...
export function GamificationSidebar() {
    ...
    return (
        <div className="space-y-6">
            {/* Player Stats */}
            <Card className="bg-slate-900/40 border-border relative overflow-hidden group">
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div className="bg-background/50 p-2 rounded-lg border border-slate-800/50">
                            <p className="text-[8px] text-muted-foreground uppercase font-black">Global Rank</p>
                            <p className="text-sm font-bold text-white">#1,284</p>
                        </div>
                        <div className="bg-background/50 p-2 rounded-lg border border-slate-800/50">
                            <p className="text-[8px] text-muted-foreground uppercase font-black">Badges</p>
                ...
                <CardContent className="p-0">
                    <div className="divide-y divide-slate-800/50">
                        {leaderboard.map((user, i) => (
                            <div key={i} className="flex items-center justify-between p-4 hover:bg-secondary/40 transition-colors group cursor-pointer">
                                <div className="flex items-center gap-3">
                                    <span className={`text-xs font-black ${i === 0 ? 'text-yellow-500' : 'text-muted-foreground'}`}>{i + 1}</span>
                                    <div className="w-8 h-8 rounded-full bg-secondary border border-border overflow-hidden" />
                                    <div>
                                        <p className="text-xs font-bold text-foreground group-hover:text-white">{user.name}</p>
                                        <p className="text-[9px] text-muted-foreground uppercase font-black">{user.role} • LVL {user.level}</p>
                                    </div>
                                </div>
                    ...
                    <Button variant="ghost" className="w-full text-[10px] font-black italic tracking-widest text-muted-foreground hover:text-primary py-4 uppercase">
            ...
            {/* Recent Achievements */}
            <div className="p-4 rounded-xl border border-border bg-gradient-to-br from-background to-background">
                <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3 flex items-center gap-2">
                    <Award className="w-3 h-3 text-foreground/70" /> Recent Unlock
                </h4>
                <div className="flex items-center gap-3">
