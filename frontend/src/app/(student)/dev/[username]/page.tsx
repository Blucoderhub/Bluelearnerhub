...
import {
  ...
  Calendar, Award, Code2, Users, Activity, Flame,
  BookOpen, Trophy, ChevronRight, Lock, Globe,
} from 'lucide-react';
...
const MOCK_REPOS = [
  {
    id: 1, name: 'turbo-cache', description: 'High-performance Redis-compatible in-memory cache written in Rust.',
    language: 'Rust', stars: 241, forks: 38, visibility: 'public',
    topics: ['rust', 'cache', 'performance'], updatedAt: '2026-03-08',
  },
  {
    id: 2, name: 'react-query-builder', description: 'Visual SQL query builder component for React applications.',
    language: 'TypeScript', stars: 189, forks: 22, visibility: 'public',
    topics: ['react', 'sql', 'ui'], updatedAt: '2026-03-06',
  },
  {
    id: 3, name: 'distributed-scheduler', description: 'Distributed cron job scheduler with Postgres-backed persistence.',
    language: 'Go', stars: 74, forks: 11, visibility: 'public',
    topics: ['go', 'cron', 'distributed'], updatedAt: '2026-02-28',
  },
  {
    id: 4, name: 'dotfiles', description: 'My personal dev environment — zsh, tmux, neovim configs.',
    language: 'Shell', stars: 8, forks: 3, visibility: 'public',
    topics: ['dotfiles', 'neovim'], updatedAt: '2026-02-10',
  },
];

const LANG_COLORS: Record<string, string> = {
  TypeScript: '#3178c6', JavaScript: '#f1e05a', Python: '#3572A5',
  Rust: '#dea584', Go: '#00ADD8', Shell: '#89e051', CSS: '#563d7c',
};
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="lg:w-64 shrink-0">
            <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <div className="flex flex-col items-center lg:items-start">
                <div className="h-24 w-24 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-3xl font-bold mb-3">
              ...
              <p className="mt-3 text-sm text-gray-400 leading-relaxed">{user.bio}</p>

              {/* Meta */}
              <div className="mt-4 space-y-1.5 text-sm text-gray-500">
                {user.location && (
                  <div className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" />{user.location}</div>
                )}
                {user.website && (
                  <div className="flex items-center gap-1.5">
                    <Link2 className="h-3.5 w-3.5" />
                    <a href={user.website} className="text-blue-400 hover:underline" target="_blank" rel="noreferrer">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  Joined {new Date(user.joinedAt).toLocaleDateString('en', { month: 'long', year: 'numeric' })}
                </div>
              </div>
              ...
              {/* XP / Level */}
              <div className="mt-5 rounded-xl bg-gray-900 border border-gray-800 p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-gray-500 uppercase tracking-wider">Level {user.level}</span>
                  <Badge className="bg-purple-900/50 text-purple-400 text-xs">{user.rank}</Badge>
                </div>
                ...
                <p className="text-xs text-gray-600 mt-1">{user.xp.toLocaleString()} XP</p>
              </div>
              ...
              {user.certificates.length > 0 && (
                <div className="mt-5">
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Certificates</p>
          ...
          <main className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href={`/dev/${user.username}/repos`} className="text-xs text-blue-400 hover:underline flex items-center gap-1">
                    ...
                    <Card className="bg-gray-900 border-gray-800 hover:border-gray-700 transition-all h-full group">
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <Badge className="text-[10px] bg-gray-800 text-gray-400 border border-gray-700 shrink-0">
                            {repo.visibility}
                          </Badge>
                        </div>
                        <p className="text-xs text-gray-500 mb-3 leading-relaxed line-clamp-2">{repo.description}</p>
