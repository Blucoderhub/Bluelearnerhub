        ...
        <div className="h-[90vh] flex flex-col gap-4 -mt-6">
            ...
            <div className="flex items-center justify-between bg-card/50 border border-border p-3 rounded-xl backdrop-blur-md">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button size="sm" className="bg-primary/90 hover:bg-primary text-white font-bold h-8 px-4 rounded-lg">
            ...
            <div className="flex-1 flex gap-4 overflow-hidden">
                ...
                <div className="w-64 bg-background/40 border border-border rounded-xl flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 p-2 space-y-1">
                        <div className="flex items-center p-2 text-xs font-bold text-muted-foreground gap-2 cursor-default">
                        ...
                        {files.map(file => (
                            <div
                                ...
                                className={`flex items-center gap-2 p-2 rounded-lg text-xs font-medium cursor-pointer transition-colors ${activeFile === file.name ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-secondary/40 hover:text-foreground'}`}
                            >
                    ...
                    <div className="p-4 bg-slate-900/40 border-t border-border">
                        <div className="flex items-center gap-3">
                            <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                            <p className="text-[9px] text-muted-foreground font-bold uppercase">Synced with GitHub</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 flex flex-col gap-4 overflow-hidden">
                    <div className="flex-1 bg-background/80 border border-border rounded-xl relative overflow-hidden group">
                        <div className="absolute top-0 left-0 w-full h-8 bg-card/60 border-b border-border flex items-center px-4 justify-between">
                            <div className="flex items-center gap-2">
                                <FileCode className="w-3.5 h-3.5 text-blue-400" />
                                <span className="text-xs font-bold text-foreground/80 uppercase">{activeFile}</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <Badge variant="outline" className="text-[10px] text-muted-foreground font-mono">UTF-8</Badge>
                                <Badge variant="outline" className="text-[10px] text-muted-foreground font-mono">Python 3.11</Badge>
                            </div>
                        </div>
                        <div className="mt-8 p-6 font-mono text-sm leading-relaxed text-foreground/80 overflow-y-auto h-full scrollbar-hide">
                            <div className="flex gap-4">
                                <div className="text-slate-700 select-none text-right w-8 border-r border-border pr-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <pre className="text-muted-foreground italic mt-2"># Bluelearner-AI specialized agent initialization</pre>
                                    <pre className="text-blue-200">model = tf.keras.Sequential([</pre>
                                    <pre className="text-blue-200">    tf.keras.layers.Dense(<span className="text-yellow-400">128</span>, activation=<span className="text-foreground/70">'relu'</span>),</pre>
                                    ...
                                    <pre className="text-blue-200">    <span className="text-muted-foreground">...</span></pre>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="absolute right-4 bottom-4 w-64 bg-slate-900/90 border border-primary/20 rounded-2xl p-4 backdrop-blur-xl shadow-2xl opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <p className="text-[10px] text-muted-foreground leading-tight">I noticed a potential optimization in your model layers. Should I refactor?</p>
                            <div className="flex gap-2 mt-3">
                                <Button size="sm" className="bg-primary/10 text-primary border-primary/20 h-7 text-[9px] font-black">REFACTOR</Button>
                                <Button size="sm" variant="secondary" className="h-7 text-[9px] font-bold text-muted-foreground">DISMISS</Button>
                            </div>
                        </div>
                    </div>

                    {/* Terminal */}
                    <div className="h-48 bg-background border border-border rounded-xl overflow-hidden flex flex-col">
                        <div className="p-2 border-b border-border bg-slate-900/40 flex items-center justify-between">
                            <div className="flex items-center gap-2 ml-2">
                                <Terminal className="w-3.5 h-3.5 text-muted-foreground" />
                                <span className="text-[10px] font-bold text-muted-foreground uppercase">TERMINAL_01</span>
                            </div>
                        ...
                        <div className="flex-1 p-4 font-mono text-xs text-muted-foreground overflow-y-auto">
                            ...
                            <div className="flex mt-1">
                                <span className="text-foreground/80 mr-2">➜</span>
                                <input className="bg-transparent border-none outline-none flex-1 text-foreground" autoFocus />
                            </div>
                        </div>
                    </div>
                </div>
