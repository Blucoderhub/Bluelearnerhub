...
export default function CategoryLibraryPage() {
  ...
  const domainName = pathId ? (pathId as string).replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : 'Domain'
  const categoryName = courseId ? (courseId as string).replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : 'Category'

  useEffect(() => {
    if (!pathId || !courseId) return
    
    tutorialsAPI.list({ domain: pathId, subDomain: courseId })
      .then((d: any) => {
        const data = d.data || d
          ...
          <div className="flex flex-col gap-10 md:flex-row md:items-end md:justify-between">
            <div className="space-y-6">
              <div className="flex items-center gap-6">
                <Badge variant="outline" className="rounded-full border-primary/30 bg-primary/10 px-4 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-primary">
                  {domainName}
                </Badge>
              ...
              <p className="max-w-xl font-serif text-xl leading-relaxed text-muted-foreground">
                Deep-dive into <span className="text-foreground">{categoryName}</span> with our expert-led tutorials and interactive project-based curriculum.
              </p>
            </div>
        ...
        <div className="space-y-8">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center">
              ...
              <p className="text-muted-foreground">Try broadening your search within this category.</p>
            </div>
          )}
        </div>
      </div>
