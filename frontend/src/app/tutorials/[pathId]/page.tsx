...
export default function DomainLibraryPage() {
  ...
  const domainName = pathId ? (DOMAIN_NAMES[pathId as string] || (pathId as string).replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())) : 'Domain'
  const DomainIcon = pathId ? (DOMAIN_ICONS[pathId as string] || BookOpen) : BookOpen

  useEffect(() => {
    if (!pathId) return
    tutorialsAPI.list({ domain: pathId })
      .then((d: any) => {
        const data = d.data || d
          ...
          <div className="flex flex-col gap-10 md:flex-row md:items-end md:justify-between">
            <div className="space-y-6">
              ...
              <p className="max-w-xl font-serif text-xl leading-relaxed text-muted-foreground">
                Master the core principles and industry-grade techniques in <span className="text-foreground">{domainName}</span>.
              </p>
            </div>
        ...
        <div className="space-y-32">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center">
              ...
              <p className="text-muted-foreground">Try adjusting your search query to find relevant tutorials.</p>
            </div>
          )}
        </div>
      </div>
