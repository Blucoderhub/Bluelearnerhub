...
export default function CandidatesPage() {
    ...
    return (
        <div className="space-y-10 pb-32">
            {/* Header & Controls */}
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-slate-900/40 p-10 rounded-3xl border border-border glass-morphism relative overflow-hidden">
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                        {domains.map(d => (
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest italic transition-all ${selectedDomain === d ? 'bg-primary text-primary-foreground ai-glow' : 'bg-background text-muted-foreground hover:text-white border border-border hover:border-slate-600'
                                    }`}
                            >
                                {d}
                            </button>
                ...
                <div className="flex gap-4 w-full md:w-auto relative z-10">
                    ...
                    <Button className="h-12 w-12 bg-background border border-border text-white hover:bg-card">
                    ...
                    >
                        <Card className="bg-card border-border hover:border-primary/30 transition-all group overflow-hidden relative">
                            <div className="absolute top-0 right-0 p-6 flex flex-col items-end gap-2">
                                ...
                                <div className="bg-background/80 border border-border px-3 py-1 rounded-full">
                                    <span className="text-[8px] font-black text-muted-foreground uppercase tracking-widest">{candidate.xp.toLocaleString()} XP</span>
                                </div>
                            </div>
                            ...
                            <CardHeader className="flex flex-row items-center gap-6 p-8 border-b border-slate-800/50">
                                <Avatar className="h-20 w-20 border-2 border-border group-hover:border-primary/50 transition-colors">
                                    <AvatarImage src="" />
                                    <AvatarFallback className="bg-secondary text-white text-2xl font-black italic">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="flex items-center gap-4 pt-2">
                                        ...
                                        <span className="text-[10px] font-black text-muted-foreground uppercase tracking-tighter flex items-center gap-1">
                                            <Building2 className="w-3 h-3 text-primary/80/50" /> {candidate.university}
                                        </span>
                                    </div>
                                </div>
                            </CardHeader>
                            ...
                            <CardContent className="p-8 space-y-8">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] italic">Skill_Efficiency_Vectors</h4>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {candidate.skills.map(skill => (
                                            <div key={skill} className="px-3 py-1.5 bg-background border border-border rounded-lg flex items-center gap-2 group-hover:border-primary/20 transition-all">
                                                <Code className="w-3 h-3 text-primary/50" />
                                                <span className="text-[9px] font-black uppercase text-muted-foreground">{skill}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {candidate.achievements.map((acc, idx) => (
                                        <div key={idx} className="p-4 bg-primary/5 border border-border rounded-xl flex items-center gap-3">
                                            <div className="p-2 bg-primary/10 rounded-lg">
                                                <CheckCircle2 className="w-4 h-4 text-foreground/70" />
                                            </div>
                                            <span className="text-[9px] font-black text-foreground/40 uppercase tracking-tighter">{acc}</span>
                                        </div>
                                    ))}
                                </div>
                            </CardContent>
                            ...
                            <CardFooter className="p-8 pt-0 flex gap-4">
                                ...
                                <Button className="flex-1 bg-background border border-border text-white hover:bg-card font-black italic uppercase tracking-widest h-12">
                                    DEEP_PROFILE_V3
                                </Button>
                            </CardFooter>
                        </Card>
            ...
            <div className="mt-20 p-10 bg-background border border-primary/20 rounded-3xl relative overflow-hidden group">
                ...
                <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
                    ...
                    <div className="space-y-4">
                        ...
                        <Button className="bg-background border border-primary/30 text-primary hover:bg-primary hover:text-white font-black italic uppercase tracking-[0.2em] text-[10px] h-10 px-8 transition-all">
