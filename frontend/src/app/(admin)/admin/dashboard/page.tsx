    ...
    <div className="space-y-12 pb-32 font-mono">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest max-w-xl leading-relaxed">
            Centralized Command for BLUELEARNERHUB Global Infrastructure. Monitoring 1.2M Concurrent User Vectors.
          </p>
        </div>
        <div className="flex gap-3">
          <Button className="bg-slate-900 border border-slate-800 text-white hover:bg-slate-800 h-14 px-8 font-black italic tracking-tight uppercase">
            SECURITY_LOGS
          </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          >
            <Card className="bg-slate-950/60 border-slate-900 hover:border-red-600/30 transition-all p-8 relative overflow-hidden group">
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-100 transition-opacity">
                <metric.icon className={`w-8 h-8 ${metric.color}`} />
              </div>
              <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2">{metric.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="bg-slate-950/40 border border-slate-900 p-8 grid grid-cols-2 md:grid-cols-4 gap-8">
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-slate-950 border-red-600/20 p-8 space-y-6 overflow-hidden relative">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex gap-4 p-4 bg-slate-900/50 rounded-xl border border-slate-800 hover:border-red-600/30 transition-all">
                  <AlertTriangle className={`w-4 h-4 shrink-0 mt-0.5 ${alert.type === 'critical' ? 'text-red-500' : 'text-orange-500'}`} />
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-white uppercase leading-tight italic">{alert.msg}</p>
                    <p className="text-[8px] text-slate-600 font-bold uppercase">{alert.time}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {[
            ...
            <button key={cmd.n} className="p-4 bg-slate-900/40 border border-slate-800 rounded-xl text-center space-y-2 hover:bg-slate-900 transition-all hover:border-red-600/20">
