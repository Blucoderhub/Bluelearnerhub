    ...
    return (
        ...
        >
            <Link href={`/courses/domain/${domain.name.toLowerCase().replace(/\s+/g, '-')}`}>
                <Card className={`group relative bg-slate-900/30 border ${domain.border} hover:bg-slate-900/60 hover:border-primary/50 transition-all duration-300 overflow-hidden cursor-pointer h-full`}>
                    <div className="absolute top-0 right-0 p-2">
                        <Badge variant="outline" className="text-[10px] uppercase font-bold text-slate-500 group-hover:text-primary transition-colors">
                            {category}
                        </Badge>
                    </div>
                    <CardHeader className="pt-8">
                        <div className={`w-12 h-12 rounded-xl bg-slate-950 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-500 border border-slate-800`}>
